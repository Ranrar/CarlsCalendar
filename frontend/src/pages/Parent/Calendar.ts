...
export async function render(container: HTMLElement): Promise<void> {
    ...
    <main class="container page-content">
      <div class="page-header">
        ...
        <div style="display:flex;gap:.75rem;align-items:center">
          <select id="child-select" class="child-select"><option value="">Select child…</option></select>
          <a class="btn btn-secondary btn-sm" href="/schedules">Manage schedules</a>
        </div>
      </div>
      <div id="week-wrap" class="week-view-wrap">
        <div class="empty-state">
          <span class="empty-state__icon">📅</span>
          <p>Select a child to view their calendar.</p>
        </div>
      </div>
    </main>`;
  ...
  try {
    ...
    children.forEach((c) => {
      const o = document.createElement('option'); o.value = c.id; o.textContent = c.display_name;
      childSelect.appendChild(o);
    });
    const p = new URLSearchParams(location.search).get('child');
    if (p) { childSelect.value = p; loadWeek(); }
  } catch { /* non-fatal */ }
  ...
  async function loadWeek(): Promise<void> {
    ...
    try {
      ...
      const data = await api.get<{ date: string; items: WeekDay['items'] }[]>(
        `/calendar?child_id=${childSelect.value}&week=${week}&year=${year}`);
      data.forEach((a) => { const day = days.find((d) => d.date === a.date); if (day) day.items = a.items; });
      renderWeekView(wrap, days, week, year, false);
