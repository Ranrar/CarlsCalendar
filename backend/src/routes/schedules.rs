    ...
    let rows: Vec<ScheduleRow> = if user.role == UserRole::Admin {
        sqlx::query_as::<_, ScheduleRow>(
            "SELECT id, owner_id, child_id, name, status, is_template
             FROM schedules WHERE is_template = 0 ORDER BY name",
        )
        .fetch_all(pool).await?
    } else {
        sqlx::query_as::<_, ScheduleRow>(
            "SELECT id, owner_id, child_id, name, status, is_template
             FROM schedules WHERE owner_id = ? AND is_template = 0 ORDER BY name",
        )
        .bind(&user.user_id)
        .fetch_all(pool).await?
    };
...
async fn copy_template(
    ...
    let tmpl: ScheduleRow = sqlx::query_as::<_, ScheduleRow>(
        "SELECT id, owner_id, child_id, name, status, is_template
